...
let win;
const store = new ElectronStore({ name: "friendscave" });
ipcMain$1.handle("store:get", (_e, key) => {
...
ipcMain$1.handle("store:has", (_e, key) => {
  return store.has(key);
});
ipcMain$1.handle("store:delete", (_e, key) => {
  store.delete(key);
});
ipcMain$1.handle("store:clear", () => {
  store.clear();
});
ipcMain$1.handle("store:keys", () => {
